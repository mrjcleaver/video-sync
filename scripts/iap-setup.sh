...
# 6. Compute the IAP_AUDIENCE for the JWT verifier.
#    Cloud Run direct IAP (no load balancer) uses a region/service-named
#    audience — NOT the global/backendServices format which is for
#    load-balancer-fronted IAP. Confirmed empirically from a real JWT:
#    aud='/projects/<num>/locations/<region>/services/<name>'.
