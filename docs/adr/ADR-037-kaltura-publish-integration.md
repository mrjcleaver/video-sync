...
- **Upload size and memory**: this took two iterations to pin down.

  - **First failure**: my initial code buffered the whole video into a `Buffer` then a `Blob` before posting. Peak RSS ~3× file size, OOM at 1 GiB → Cloud Run 503. Fix: `node:fs.openAsBlob(path)` returns a lazy Blob; undici streams from disk during multipart serialisation, peak memory ~64 KB. Worked for files small enough that the temp file itself fit in RAM.

  - **Second failure**: turned out the *upload* path was no longer the problem — the *download* path was. The route writes the source media to `/tmp/kaltura-upload-*.mp4` via `pipeline()` before opening it. **On Cloud Run, `/tmp` is tmpfs (RAM-backed)**: a 2 GB video sitting in `/tmp` consumes 2 GB of memory. Plus Node's working set (~200 MB) plus undici buffers, the 1 GiB instance ran out of room and the fetch to Kaltura was aborted internally — surfacing as `Kaltura: terminated`. The container itself didn't OOM-kill (handler still ran and returned 502), but the upload connection was torn down before completion.

  - **Fix landed 2026-04-22**: bumped Cloud Run memory to **4 GiB** (and CPU to 2 — Cloud Run requires `cpu>=2` at 4 GiB memory). `MEMORY_LIMIT_MB` env var updated to 4096 so the runtime memory monitor (ADR-032) reports against the new ceiling. This works for typical 1–2 hour livestreams (file ~1–2 GB, leaves >2 GB headroom for Node).

  - **Phase 2 still required** for files larger than the temp-file-in-RAM model can hold (multi-GB). The right fix is **stream-through**: open a fetch to the source (Zoom/Fireflies/Loom), wrap its `ReadableStream` as the multipart body, never write to disk. Or use Kaltura's chunked upload (`resumeAt=N`) so each chunk is independently uploaded and the temp file can be replaced by a streaming pipe. Both deferred.
