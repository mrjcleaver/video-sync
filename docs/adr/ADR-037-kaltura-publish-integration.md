...
- **Upload size**: single-request multipart for files >2 GB hits Cloud Run's 32 MB **request limit only when the request body originates client-side**. Our upload route downloads the source media server-side first, then posts to Kaltura — so the client→Cloud-Run request body is just the JSON credentials, not the video. The video flows from Cloud Run → Kaltura which has no 32 MB cap. The actual size constraint is Cloud Run's **memory limit** (currently 1 GiB): an early version of this code buffered the whole file into a `Buffer` then a `Blob` before posting, peak RSS ~3× file size, OOM-killed by Cloud Run on 1+ GB videos and the client got a 503. Fix landed 2026-04-22: use `node:fs.openAsBlob(path)` which returns a lazy Blob; undici streams from disk during multipart serialisation, peak memory ~64 KB. Mitigation for files larger than what single-shot upload can handle reliably (multi-GB): switch to Kaltura's chunked upload (`resumeAt=N` per chunk) when adding SSE in Phase 2.
