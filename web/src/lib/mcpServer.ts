...
async function methodResourcesRead(actor: Actor, params: unknown) {
  ...
  if (parsed.kind === "description-full") {
    ...
    // No precomputed artifact. description-full is LLM-generated
    // (same shape as description, un-capped). Rather than generate
    // it here on-demand (LLM roundtrip, wrong-shape deterministic
    // fallback), tell the caller how to have it materialised.
    throw new McpError(ERR_INVALID_PARAMS, "description-full.md not yet generated for this record — open the video card and click 'Rewrite from Show Notes' to have it written.");
  }
...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Return the full-length variant of the record's description — same SHAPE as the shipped description (opening hook + chapter cues + highlights) but generated by the LLM without the 5000-char YouTube cap. Written to description-full.md at Rewrite-from-Show-Notes time. Returns an informative error when not yet materialised.",
    inputSchema: { type: "object", properties: { record_id: { type: "string" } }, required: ["record_id"] },
  },
] as const;
...
async function toolGetDescriptionFull(actor: Actor, args: Record<string, unknown>) {
  ...
  // description-full is LLM-generated (same shape as description,
  // un-capped) at Rewrite-from-Show-Notes time. When absent, tell
  // the caller how to have it materialised rather than serving a
  // wrong-shape fallback.
  return textResult(
    "description-full.md not yet generated for this record — open the video card and click 'Rewrite from Show Notes' to have it written.",
    true,
  );
}
